...
CONFIG.fossil_name = 'Pipeline';

// the starting view: bounds, see initMap()
// this is an arbitrary bounds, preferred by the client
CONFIG.homebounds = [[65, 125], [-30,-115]];

// minzoom and maxzoom for the map
CONFIG.minzoom = 2;
CONFIG.maxzoom = 15;

// min and max circle radius, for the capacity map
CONFIG.minradius = 4;
CONFIG.maxradius = 30;
CONFIG.nodata_radius = 2;
...
// primary use will be in the DataTable and the popup
CONFIG.attributes = {
...
// break the popup into two cols with this many rows in each column
CONFIG.number_of_rows_per_popup_column = 4;
...
//          key: allowed status names, matching those used in DATA.rawdata
...
CONFIG.status_types = {
  ...
  'construction': {text: 'Construction', color: styles.status2, cssclass: 'status2', order: 2 },
  'proposed': {text: 'Proposed', color: styles.status3, cssclass: 'status3', order: 3 },
  'cancelled': {text: 'Cancelled', color: styles.status4, cssclass: 'status4', order: 4 },
  'shelved': {text: 'Shelved', color: styles.status5, cssclass: 'status5', order: 5 },
  'retired': {text: 'Retired', color: styles.status6, cssclass: 'status6', order: 6 },
  'mothballed': {text: 'Mothballed', color: styles.status7, cssclass: 'status7', order: 7 },
  'idle': {text: 'Idle', color: styles.status8, cssclass: 'status8', order: 8 },
};
...
$(document).ready(function () {
  ...
  Promise.all([
      ...
      initMapViewControl();   // init the capacity view switcher
...
function completeFinalInitSteps(fitmap=false) {
  setTimeout(function() {
    resize();
    if (fitmap) CONFIG.map.fitBounds(CONFIG.homebounds);
    $('div#loading').hide();
...
function initButtons() {
  ...
  // init the behavior of the "Show all phases/units" button on the Tracker Info Panel
  // in short, this triggers a custom "search" on the project name, redraws the map, and zooms to the results
  $('a#multiple-units').on('click', function() {
    // set the previous data, the name of the filtered data object, and a map draw function, so we can go back to previous state
    CONFIG.backbutton.setPreviousData(DATA.filtered, drawMap);
    
    // get the target project that opened this info panel
    CONFIG.backbutton.setTargetTracker($(this).data().tracker);

    // get the project name and search for it
    let project_name = this.dataset.name;
    DATA.filtered = DATA.rawdata.features.filter(function(d) {return d.properties.project == project_name});

    // set the previous bounds that we should go to when we click the back button
    CONFIG.backbutton.setPreviousBounds(CONFIG.map.getBounds());

    // set the previous results message
    CONFIG.backbutton.setPreviousResultsMessage($('div#results-title h3').text());

    // set the current set of status checkboxes 
    let statuses = $('div#status-types input:checkbox:checked').map(function() {
      return this.dataset.val;
    }).get();
    CONFIG.backbutton.setStatusChecks(statuses);

    // set the current set of type checkboxes 
    let types = $('div#fossil-types input:checkbox:checked').map(function() {
      return this.dataset.val;
    }).get();
    CONFIG.backbutton.setTypeChecks(types);

    // add the back button, which takes the user back to previous view (see function goBack()
    // but only if there is not already a back button on the map
    if ($('.btn-back').length == 0) CONFIG.backbutton.addTo(CONFIG.map);

    drawMap(false);                                        // always update the map, but don't filter based on checkbox state
    updateResultsPanel(project_name);                      // update the results-panel
    ...
    // zoom to the results, and hide the modal
    CONFIG.map.fitBounds(L.geoJson(DATA.filtered).getBounds());
    $('div#tracker-modal').modal('hide');
    updateStateParams();                                 
  });
}
...
// initialize the map switcher control
// in other trackers, this is a proper L.Control, not here because of the additional complexity
function initMapViewControl() {
  $('div.leaflet-control-mapswitcher-option').on('click', function() {
    let button = $(this);
    // State managment: if already selected, do nothing
    if (button.hasClass('leaflet-control-mapswitcher-option-active')) return;

    // Make this button active
    $('div.leaflet-control-mapswitcher-option').removeClass('leaflet-control-mapswitcher-option-active');
    button.addClass('leaflet-control-mapswitcher-option-active');

    // if going from capacity view to project view,
    // sync the selected radio from capacity view to the corresponding checkbox in plant view
    if (button.data().layer == 'plantview') {
      let selected_type = $('input[name="type-capacity"]:checked').val();
      $('div#fossil-types-project input').prop('checked', false);
      $(`div#fossil-types-project input[value="${selected_type}"]`).prop('checked', true);
    }
...
function updateResultsPanel(title=CONFIG.default_title, countrysearch=false) {
  ...
  // for each status type, get the count of that status from the data
  Object.keys(CONFIG.status_types).forEach(function(type) {
    let count = 0;
    ...
    let counttext = CONFIG.format.number(count);
    let label = CONFIG.status_types[type].text;
    let html = `${label}<span>${counttext}</span>`;
...
function drawTable(data=DATA.rawdata.features, title) {
  ...
  const tabletitle = typeof name == 'undefined' ? 'All records' : 'Records for ' + name;
  $('#table h2 span').text(tabletitle);
}
...
function getResultsMessage() {
  ...
  let typecount = $('div#fossil-types-project input').length;
  let typechecked = $('div#fossil-types-project input:checked').length;
...
function openTrackerInfoPanel(feature) {
  ...
  Object.keys(CONFIG.attributes).forEach(function(attr, i) {
    ...
    const index = i > CONFIG.number_of_rows_per_popup_column ? 2 : 1;
    ...
    $('<span>', {class: 'key bold', text: `${key}: `} ).appendTo(outer);
    $('<span>', {class: 'value', text: value} ).appendTo(outer);
  });

  // if this project has multiple units, show the "multi-unit" button
  // otherwise hide it
  if (properties.unit_count > 1) {
    $('a#multiple-units').attr('data-name', properties.project).show();
  } else {
    $('a#multiple-units').hide();
  }
  ...
  // ...and similar behavior for the "multiple unit/phase" button
  $('a#multiple-units').data().tracker = feature;
